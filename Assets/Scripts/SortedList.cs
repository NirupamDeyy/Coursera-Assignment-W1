...
public class SortedList<T> where T:IComparable
{
    ...
    public void Add(T item)
    {
        ...
        // Copy the items from the original list to tempList.
        tempList.AddRange(items);

        // Find the index to insert the item using binary search.
        int index = tempList.BinarySearch(item);
        if (index < 0)
        {
            index = ~index; // Convert the binary search result to a valid insertion point.
        }

        // Insert the item at the appropriate position in tempList.
        tempList.Insert(index, item);

        // Copy the sorted tempList back to the original list.
        items.Clear();
        items.AddRange(tempList);

        // Clear tempList for future use.
        tempList.Clear();
